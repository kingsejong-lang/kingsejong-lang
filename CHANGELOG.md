...
### Added
- **ErrorReporter 시스템 통합**:
  - main.cpp의 executeFile() 함수에서 ErrorReporter 사용
  - REPL의 evalAndPrint() 함수에서 ErrorReporter 사용
  - 기본 힌트 시스템 자동 등록 (registerDefaultHints)

### Changed
- **개선된 에러 메시지**:
  - 한글 에러 타입 표시: 구문 오류, 실행 오류, 타입 오류 등
  - ANSI 컬러 출력: 빨강(에러), 청록(힌트), 흐림(줄 번호)
  - 20+ 에러 패턴에 대한 도움말/힌트 제공
  - 소스 파일 등록 및 추적 (파일명 표시)

### Testing
- **전체 테스트 729개 (100% 통과)**
- ErrorReporter 통합 후 모든 테스트 정상 작동

### Example
**Before:**
```
파서 에러:
  다음 토큰으로 'IDENTIFIER'을(를) 예상했지만, 'ASSIGN'을(를) 받았습니다.
파일: test.ksj
```

**After:**
```
❌ 구문 오류: 다음 토큰으로 'IDENTIFIER'을(를) 예상했지만, 'ASSIGN'을(를) 받았습니다.
