...
### F5.NEW: 에러 메시지 개선 ⭐⭐⭐
- 상태: ✅ **완료** (2025-11-12)
- 브랜치: `feature/f5-new-error-messages` → `main`
- **우선순위**: CRITICAL
- **실제 기간**: 1일 (통합 작업)
...
- **실제 기간**: 1일 (통합 작업)
- **작업**:
  - [x] ErrorReporter 클래스 설계 (이전 세션 완료)
  - [x] 한글 에러 메시지 체계 구축
  - [x] 에러 위치 표시 (줄, 컬럼) - SourceLocation 지원
  - [x] 소스 코드 컨텍스트 표시 (전후 줄)
  - [x] 에러 유형별 해결 방법 힌트 (20+ 패턴)
  - [x] 스택 트레이스 개선 (구조 설계 완료)
  - [x] 컬러 출력 지원 (ANSI 컬러)
  - [x] main.cpp와 REPL에 통합
- **에러 유형 커버리지**:
  - [x] 구문 에러 (파싱 실패) - ParserError
  - [x] 타입 에러 (타입 불일치) - TypeError
  - [x] 런타임 에러 (실행 중 에러) - RuntimeError
  - [x] 이름 에러 (정의되지 않은 변수) - NameError
  - [x] 인덱스 에러 (배열 범위 초과) - IndexError
  - [x] 0으로 나누기 에러 - ZeroDivisionError
  - [x] 인자 에러 - ArgumentError
  - [x] 값 에러 - ValueError
- **테스트**:
  - [x] ErrorReporter 테스트 (7개 추가)
  - [x] 전체 테스트 729개 통과 (100%)
- **완료 조건**:
  - [x] 모든 에러가 한글로 표시
  - [x] 에러 타입별 한글 변환
  - [x] 도움말 힌트 제공
...
#### F5.3.2: 고급 LSP 기능 ✅
- **상태**: ✅ **완료** (2025-11-14 완료)
- **브랜치**: `feature/lsp-advanced` → `main`
- **실제 기간**: 2일 (2025-11-13 ~ 2025-11-14)
- **우선순위**: HIGH
- **목표**: IDE 고급 기능 제공 (정의 이동, 참조 찾기 등) ✅
- **PR**: #53
- **릴리스**: v0.3.2 (예정)
- **작업**:
  - [x] SymbolTable 구현 (2025-11-12 완료)
    - 심볼 저장 및 조회
    - 스코프 기반 검색
    - 위치 기반 검색
    - 참조 추적
    - **테스트**: 15/15 통과 ✅
  - [x] SymbolCollector 구현 (2025-11-13 완료)
    - AST 기반 심볼 수집
    - 변수/함수 선언 추적
    - 매개변수 및 참조 수집
    - 스코프 관리
    - **테스트**: 10/10 통과 ✅
  - [x] 정의로 이동 (Go to Definition) (2025-11-13 완료)
    - textDocument/definition 메서드 구현
    - 커서 위치 심볼 찾기 (LspUtils - UTF-16/UTF-8 변환)
    - 스코프 인식 (전역/로컬 변수 구분)
    - 정의 위치 반환
    - **테스트**: 9/9 통과 ✅ (추가로 LspUtils 7개, Integration 4개)
  - [x] 호버 정보 (Hover Information) (2025-11-14 완료)
    - textDocument/hover 메서드 구현
    - 타입 정보 표시 (변수, 함수, 매개변수)
    - 함수 시그니처 표시
    - 스코프 인식 (로컬/전역)
    - **테스트**: 8/8 통과 ✅
  - [x] 참조 찾기 (Find References) (2025-11-14 완료)
    - textDocument/references 메서드 구현
    - 심볼 사용처 검색
    - includeDeclaration 파라미터 지원
    - 스코프 필터링 (로컬/전역 변수 구분)
    - **테스트**: 10/10 통과 ✅
  - [x] 이름 변경 (Rename) (2025-11-14 완료)
    - textDocument/rename 메서드 구현
    - WorkspaceEdit 생성 (정의 + 모든 참조)
    - 스코프 필터링 (로컬/전역 변수 구분)
    - 빈 이름 검증
    - **테스트**: 11/11 통과 ✅
- **테스트 현황**:
  - [x] SymbolTable 테스트 (15/15) ✅
  - [x] SymbolCollector 테스트 (10/10) ✅
  - [x] LspUtils 테스트 (7/7) ✅
  - [x] Integration 테스트 (4/4) ✅
  - [x] Go to Definition 테스트 (9/9) ✅
  - [x] Hover Information 테스트 (8/8) ✅
  - [x] Find References 테스트 (10/10) ✅
  - [x] Rename 테스트 (11/11) ✅
  - **전체 프로젝트**: 803/803 테스트 통과 ✅
- **완료 조건**:
  - [x] 모든 고급 LSP 기능 구현 완료 ✅
  - [x] 스코프 인식 (전역/로컬 변수 구분) ✅
  - [x] UTF-16/UTF-8 변환 (한글 문자 지원) ✅
  - [x] 모든 테스트 통과 (803/803) ✅
  - [x] main 브랜치 병합 완료 ✅
- **현재 상태**: ✅ **고급 LSP 기능 완료!** (2025-11-14)
...
#### 현재 상태
- **현재**: stdlib/math.ksj, stdlib/string.ksj, stdlib/array.ksj, **stdlib/io.ksj** (70개+ 함수)
- **목표**: 6개 모듈, 100개+ 함수

#### 작업 항목
- **기간**: 3주
- **작업**:
  - [x] stdlib/io.ksj - 파일 I/O (10개 builtin + 15개 고수준 함수) ✅ 2025-11-14
    - ✅ 파일_읽기(), 파일_쓰기(), 파일_추가(), 파일_존재()
    - ✅ 줄별_읽기(), 파일_삭제(), 파일_크기(), 파일_줄수()
    - ✅ 디렉토리_생성(), 디렉토리_삭제(), 디렉토리_목록(), 현재_디렉토리()
    - ✅ 고수준 함수: 안전하게_읽기(), 줄별_쓰기(), 파일_복사() 등
...
- **테스트**:
  - [x] 파일 I/O 테스트 (16개) ✅ 825/825 통과
  - [ ] HTTP 테스트 (15개)
  - [ ] JSON 테스트 (10개)
  - [ ] 시간 테스트 (15개)
  - [ ] 정규식 테스트 (10개)
- **문서**:
  - [x] stdlib/io.ksj 레퍼런스 ✅
  - [x] stdlib/io.ksj 사용 예제 ✅
  - [ ] 표준 라이브러리 통합 레퍼런스
- **완료 조건**:
  - [ ] 100개+ 함수 달성 (현재 70개+)
  - [ ] 모든 테스트 통과
...
### M5.2: 생태계 구축 (Week 16)
- [x] F5.3.2 고급 LSP 완성 ✅ (2025-11-14)
- [ ] F5.4 온라인 플레이그라운드 완성
- [ ] F5.NEW 표준 라이브러리 100개+ 함수 달성
- [x] VS Code 확장 프로그램 출시 ✅ (기본 + 고급 기능)
- [ ] 웹 배포 완료
...
### 테스트 목표
- **테스트 커버리지**: ≥ 85%
- **에러 메시지 테스트**: 45개+ ✅ (7개 완료)
- **디버거 테스트**: 30개+ (대기)
- **LSP 테스트**: 50개+ ✅ (130개 완료)
  - LSP 기본: 56개 ✅
  - LSP 고급: 74개 ✅
- **표준 라이브러리 테스트**: 70개+ (대기)
- **전체 테스트**: 750개+ ✅ (803개 달성, 목표 초과)
