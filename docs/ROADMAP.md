...
- [ ] **예외 처리 시스템 구현** ⭐⭐⭐⭐⭐ 🚧
  ...
  - [ ] Error Value 타입
  - [ ] TryStatement, ThrowStatement AST 노드
  - [ ] Parser 구현 (시도/오류/마지막/던지다 문법)
  - [ ] Evaluator 예외 처리 로직
  - [ ] 런타임 에러 복구 메커니즘
  - [ ] 테스트 케이스
  - [ ] 예상 공수: 2주
...
- [ ] **샌드박스 시스템 구현** ⭐⭐⭐⭐⭐
  - [ ] **문제**: 무제한 파일 접근 (모든 파일 읽기/쓰기/삭제 가능)
  - [ ] SecurityManager 클래스
  - [ ] 파일 경로 화이트리스트/블랙리스트
  - [ ] 권한 레벨 시스템 (TRUSTED, UNTRUSTED, SANDBOX)
  - [ ] 파일 작업 검증 (`파일_읽기`, `파일_쓰기`, `파일_삭제` 등)
  - [ ] 디렉토리 접근 제한
  - [ ] 예상 공수: 1-2주
  ```cpp
  // 예시: 샌드박스 모드에서 제한
...
- [x] **프로파일러 구현** ⭐⭐⭐⭐ ✅
  ...
  - [x] 텍스트/JSON 보고서
  - [x] 8개 테스트 통과
- [ ] **Hot Path Detector 완성** ⭐⭐⭐⭐
  - [ ] 현재 상태: 인프라만 구현됨 (HotPathDetector.cpp)
  - [ ] 함수 호출 빈도 추적
  - [ ] 루프 백에지 카운팅
  - [ ] "hot" 코드 경로 식별
  - [ ] JIT 컴파일 우선순위 결정
...
[진행중] Phase 6.3 P0-1: Semantic Analyzer 완전 구현 ✅
[진행중] Phase 6.3 P0-2: 변수 스코프 버그 수정 ✅
[진행중] Phase 6.3 P0-2: 예외 처리 시스템 (키워드만 완료)
[대기] Phase 6.3 P0-3: 보안 & 안정성 (샌드박스, 접근 제어)
...
- ✅ **P0 Phase 2 부분 완료** (변수 스코프 버그 수정, 예외 처리 키워드 추가)
...
**진행중**:
1. **예외 처리 시스템** (P0 Phase 2) - 키워드만 완료, AST/Parser/Evaluator 구현 필요
2. **ROADMAP.md 업데이트** - VM 분석 기반 보안/성능 계획 추가

**다음 우선순위** (Production Readiness 향상):
1. **예외 처리 완성** (P0) - 2주 예상
2. **보안 & 안정성** (P0) - 샌드박스, 접근 제어, 리소스 할당량 (3-4주)
3. **JIT Tier 1** (P0) - 성능 10-20배 향상 (1-2개월)
4. 딕셔너리, 배열 연산자, 문자열 보간 (P1)
5. 패키지 관리자 (Phase 6.5)

**치명적 약점 (프로덕션 블로커)**:
- 🔴 보안: 샌드박스 없음, 무제한 파일/네트워크 접근
- 🔴 예외 처리: try/catch 없음
