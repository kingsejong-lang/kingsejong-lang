...
| **VM/인터프리터 품질** | 7/10 | 🟢 양호 | JIT 최적화 필요 |
| **표준 라이브러리** | 7/10 | 🟢 양호 | 255개 함수 완료 |
| **메모리 관리** | 7/10 | 🟢 양호 | GC 작동 (세대별 미구현) |
| **성능** | 4/10 | 🔴 취약 | JIT 없음 (V8 대비 1%) |
...
#### 6.4: JIT Compiler (Tier 1) - P0 🔴 **최우선 과제**

**목표**: 10-20배 성능 향상
**예상 공수**: 1-2개월
**현재 상태**: Hot Path Detector 완료 ✅ (10개 테스트)

**구현 계획**:

1. **JIT 아키텍처 설계** (1주)
   - [ ] Tier 1 JIT 컴파일러 설계
   - [ ] 핫 패스 임계값 정의 (예: 100회 반복)
   - [ ] 네이티브 코드 생성 전략 (asmjit 라이브러리 검토)
   - [ ] Fallback 메커니즘 (JIT 실패 시 인터프리터)
