...
### 진행 중
- 🚧 Phase 5: 개발자 경험 & 생태계 구축 (0% 완료)
- 🚧 **우선순위 재조정 완료** (2025-11-11)
- 🚧 F5.NEW: 에러 메시지 개선 (시작 예정)

### 다음 작업 (재조정됨 - 개발자 경험 우선)
1. **F5.NEW: 에러 메시지 개선** (CRITICAL) - 한글 에러, 위치 표시, 힌트 (2주)
2. **F5.2.1: 기본 디버거** (CRITICAL) - 브레이크포인트, 단계별 실행 (2주)
3. **F5.2.2: 고급 디버거** (HIGH) - 조건부 BP, 와치포인트 (2주)
4. **F5.3.1: 기본 LSP** (CRITICAL) - VS Code 통합, 자동 완성 (2주)
5. **F5.3.2: 고급 LSP** (HIGH) - 정의 이동, 참조 찾기 (2주)
6. **F5.4: 온라인 플레이그라운드** (HIGH) - WebAssembly (3주)
7. **F5.NEW: 표준 라이브러리 확장** (HIGH) - 100개+ 함수 (3주)
8. **F5.5: 패턴 매칭** (MEDIUM) - 언어 기능 강화 (3주)
9. **F5.1: 템플릿 JIT 프로토타입** (LOW, 선택) - 프로토타입만 (1-2주)
